...
class RecordingStartConfirmationDelegate extends WatchUi.ConfirmationDelegate {
    ...
    function initialize(screenManager, activityType) {
        ...
        System.println("DELEGATE: RecordingStartConfirmationDelegate initialized for: " + activityType);
    }
    
    function onResponse(response) {
        System.println("INPUT: Recording start confirmation response: " + response);
        
        if (response == WatchUi.CONFIRM_NO) {
            System.println("INPUT: User cancelled - returning to activity selection");
            return false; // Let the confirmation dialog handle the dismissal
        } 
        else {
            System.println("EVENT: User confirmed - starting " + _activityType + " recording");
